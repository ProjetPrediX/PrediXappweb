...
# model_path_import = "C:\\Users\\LENOVO\\OneDrive\\Bureau\\pythonProject11\\Local.xgb"
...
def register():
    ...
    email = st.text_input("E-mail (Inscription)", key="register_email")
    password = st.text_input("Mot de passe (Inscription)", type="password", key="register_password")
    confirm_password = st.text_input("Confirmer le mot de passe (Inscription)", type="password", key="register_confirm_password")

    if st.button("S'inscrire"):
        if password == confirm_password:
            # Enregistrer les informations d'authentification dans la base de données
            # ...
...
def login():
    st.title("Connexion")
    email = st.text_input("E-mail (Connexion)", key="login_email")
    password = st.text_input("Mot de passe (Connexion)", type="password", key="login_password")

    if st.button("Se connecter"):
        # Vérifier les informations d'authentification
        if authenticate(email, password):
            st.success("Connexion réussie.")
            # Afficher la page d'accueil après une connexion réussie
            home()
        else:
...
def main():
    st.title("Mon application Predix")

    # Afficher la page d'inscription ou de connexion en fonction de l'action choisie par l'utilisateur
    action = st.sidebar.selectbox("Action", ["S'inscrire", "Se connecter"])
