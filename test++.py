...
def main():
    # Code de configuration Streamlit (titre, barre latérale, etc.)

    # Sélection de la page
    page = st.sidebar.selectbox("Page", ["Accueil", "Login", "Signup", "Auth"])
...
def main():
    ...
    if file is not None:
        ...
        if st.button("Effectuer les prédictions", key="predict_button"):
            ...
            df_cleaned['Tps_dappro'] = df_cleaned[['Tps_dappro']].apply(moy, axis=1)
            scaler = MinMaxScaler()
...
# 2. Contenu de la page Accueil
             ...
             elif selected3 == 'Accueil':
                 home()
             elif selected3 == 'Déconnexion':
                 logout()
# 3. Contenu de la page Achat Import
             elif selected3 == "Achat Import":
                 st.write("Page Achat Import - Prédictions liées à l'achat import")
                 st.markdown(open('C:\\Users\\hp\\Desktop\\import.ipynb').read())

             # 4. Contenu de la page Achat Local
             elif selected3 == "Achat Local":
                 st.write("Page Achat Local - Prédictions liées à l'achat local")
                 st.markdown(open('C:\\Users\\hp\\Desktop\\LocaL.ipynb').read())

if __name__ == "__main__":
    page = st.sidebar.selectbox("Page", ["Login", "Signup", "Auth"])
